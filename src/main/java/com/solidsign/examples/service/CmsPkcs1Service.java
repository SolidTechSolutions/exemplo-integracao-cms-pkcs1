 ...
 *
 * [ES]    Servicio para firma CAdES (CMS) en dos pasos con PKCS#1 (clave privada externa).
 ...
 */
@Service
public class CmsPkcs1Service {

    private static final Logger LOGGER = LoggerFactory.getLogger(CmsPkcs1Service.class);
    ...
    /**
     * [EN]    Sends documents and the signer certificate to SolidSign sign-preparation endpoint.
     *         Returns hashes and finalNonce for the browser extension to sign.
     *
     * [PT-BR] Envia documentos e o certificado do assinante para o endpoint sign-preparation do SolidSign.
     *         Retorna hashes e finalNonce para a extensão do browser assinar.
     *
     * [ES]    Envía documentos y el certificado del firmante al endpoint sign-preparation de SolidSign.
    ...
    public PreparedHashesResponse prepareSignature(MultipartFile[] documents) throws IOException {
        ...
        String prepUrl = baseUrl + "/solidsign/dsig/cms/pkcs1/sign-preparation";
        ...
        try {
            ...
            if (resp.getStatusCode() == HttpStatus.OK) {
                LOGGER.info("CMS PKCS1 preparation OK. finalNonce={}", resp.getBody().finalNonce);
                return resp.getBody();
            }
        } catch (HttpStatusCodeException e) {
            LOGGER.error("SolidSign prep error {}: {}", e.getStatusCode(), e.getResponseBodyAsString());
        } catch (Exception e) {
            LOGGER.error("Unexpected error during CMS preparation: {}", e.getMessage(), e);
        }
        return null;
    }
    ...
    public SignResponse finalizeSignature(Map<String, String> allParams) {
        ...
        String finalUrl = baseUrl + "/solidsign/dsig/cms/pkcs1/sign-finalization";
        ...
        try {
            ...
            if (resp.getStatusCode() == HttpStatus.OK) {
                LOGGER.info("CMS PKCS1 finalization OK.");
                return resp.getBody();
            }
        } catch (HttpStatusCodeException e) {
            LOGGER.error("SolidSign final error {}: {}", e.getStatusCode(), e.getResponseBodyAsString());
        } catch (Exception e) {
            LOGGER.error("Unexpected error during CMS finalization: {}", e.getMessage(), e);
        }
        return null;
    }

...
import com.solidsign.examples.service.CmsPkcs1Service;
 ...
 *
 * [PT-BR] Controller REST para assinatura CAdES (CMS) em dois passos com PKCS#1 (chave privada externa).
 ...
 *
 * [ES]    Controller REST para firma CAdES (CMS) en dos pasos con PKCS#1 (clave privada externa).
...
public class CmsPkcs1Controller {

    private static final Logger LOGGER = LoggerFactory.getLogger(CmsPkcs1Controller.class);

    @Autowired
    private CmsPkcs1Service service;

    /**
     * [EN]    STEP 1 — Receives document(s) and returns hashes + finalNonce for the browser extension.
     * [PT-BR] PASSO 1 — Recebe documento(s) e retorna hashes + finalNonce para a extensão do browser.
     * [ES]    PASO 1 — Recibe documento(s) y devuelve hashes + finalNonce para la extensión del navegador.
     */
    @PostMapping("/prepare")
    public ResponseEntity<PreparedHashesResponse> prepare(
            @RequestParam("document") MultipartFile[] documents) throws IOException {
        LOGGER.info("CMS PKCS1 preparation request for {} document(s).", documents.length);
